...
- [x] 环境变量配置（`NEXT_PUBLIC_TDT_KEY`、`NEXT_PUBLIC_AMAP_KEY`）
- [x] 高德路线规划 API 接入（真实道路路线）
- [x] 路线渲染（发光线条 + 起终点标记）
- [x] 路线信息面板（距离、预计时间）
...
- **参数**：
  ...
  - `key`：高德 Web 服务 key
- **返回数据解析**：
  - `route.paths[0].steps`：路线分段
  - 每个 step 的 `polyline`：坐标点字符串（;分隔）
- **工具函数**：`lib/amap.ts`

```typescript
// 调用示例
const result = await getDrivingRoute(
  { lng: 116.358, lat: 27.948 }, // 抚州
  { lng: 115.858, lat: 28.682 }  // 南昌
)
// result.path: RoutePoint[] - 路线坐标点数组
// result.distance: number - 距离（米）
// result.duration: number - 时间（秒）
```

#### 3.2) 路线渲染样式 ✅
- **线条**：`PolylineGlowMaterialProperty`
  - 颜色：青色 `#00ffff`（卫星图上醒目）
  - 宽度：10px
  - 发光强度：0.4
  - 贴地：`clampToGround: true`
- **起点标记**：亮绿色 `#00ff88`，16px
- **终点标记**：橙色 `#ff6600`，16px
- **文字标签**：黄色 `#ffff00`，18px 粗体，黑色描边
- **信息面板**：左上角显示距离和预计时间
...
**当前实现（静态路线）**：
- 调用高德 API 获取路径点数组
- 转为 `Cartesian3` 坐标：`Cartesian3.fromDegrees(lng, lat)`
- 用 `Entity.polyline` + `PolylineGlowMaterialProperty` 渲染
...
apps/sun/
├── .env.local                  # 环境变量
│                               #   NEXT_PUBLIC_TDT_KEY=xxx（天地图）
│                               #   NEXT_PUBLIC_AMAP_KEY=xxx（高德）
├── scripts/
│   └── copy-cesium.js          # Cesium 静态资源拷贝脚本
├── public/
│   └── cesium/                 # Cesium 静态资源（自动生成，已 gitignore）
├── lib/
│   └── amap.ts                 # 高德路线规划 API 工具
├── components/
│   ├── MapViewer.tsx           # Cesium Viewer 封装 + 路线渲染
